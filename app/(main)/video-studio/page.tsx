...
const VideoStudio: React.FC = () => {
    ...
    try {
      ...
      if (response.success) {
        const videoTasks = response.data.tasks.filter(t =>
          [TaskType.VIDEO_MOTION, TaskType.VIDEO_GENERATION, TaskType.VIDEO_LIPSYNC].includes(t.type as TaskType)
        )
        setTasks(videoTasks)
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Demo implementation
      await generateVideo(prompt, referenceAssets, videoFile?.file, audioFile?.file, { aspectRatio, resolution: '720p' })
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="font-bold text-xs text-white uppercase tracking-wide">{currentTaskInfo.label}</div>
            ...
            <>
                  ...
                  {videoFile && (
                    <button
                      onClick={() => {
                        setIsAnalyzing(true)
                        setTimeout(() => {
                          setPrompt((prev) => prev ? prev : "Cinematic shot derived from video analysis...")
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-zinc-400 flex items-center gap-2 uppercase tracking-widest border-b border-zinc-800 pb-2">
                  ...
                  <span className="text-zinc-500 text-[9px] font-normal ml-auto px-2 py-0.5 bg-zinc-900 rounded border border-zinc-800">{referenceAssets.length}/3 Selected</span>
                </label>
                <div className="bg-zinc-900/30 rounded-sm border border-zinc-800 overflow-hidden">
                  <div className="flex border-b border-zinc-800">
                    <button
                      onClick={() => setRefTab('upload')}
                      className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-wider transition-colors ${refTab === 'upload' ? 'bg-white/5 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                    >
                      Upload Assets
                    </button>
                    <button
                      onClick={() => setRefTab('generate')}
                      className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-wider transition-colors ${refTab === 'generate' ? 'bg-white/5 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                    >
                      AI Generator
                    </button>
                  </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {referenceAssets.filter(a => a.source === 'upload').map((asset) => (
                            <div key={asset.id} className="relative group aspect-square rounded-sm overflow-hidden border border-indigo-500/30">
                          ...
                          {generatedCandidates.map((asset) => {
                            ...
                            return (
                              <div
                                key={asset.id}
                                onClick={() => toggleReferenceAsset(asset)}
                                className={`relative cursor-pointer group aspect-square rounded-sm overflow-hidden border-2 transition-all ${isSelected ? 'border-indigo-500' : 'border-transparent hover:border-zinc-700'}`}
                              >
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={asset.url} alt="" className="w-full h-full object-cover" />
                                {isSelected && (
              ...
              {/* Audio Track */}
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-zinc-400 flex items-center gap-2 uppercase tracking-widest border-b border-zinc-800 pb-2">Audio_Track</label>
                <FileUpload
                  type="audio"
                  accept="audio/*"
              ...
              {/* Video Prompt */}
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-zinc-400 flex items-center gap-2 uppercase tracking-widest border-b border-zinc-800 pb-2">Video Prompt</label>
                <textarea
                  value={prompt}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  <div className={`space-y-2 ${hasReferenceAssets ? 'opacity-50 pointer-events-none' : ''}`}>
                    <label className="text-[9px] text-zinc-600 uppercase tracking-wider font-mono">Aspect Ratio</label>
      ...
      <div className="flex-1 bg-black flex flex-col h-full relative z-10 overflow-hidden">
        ...
        <div className="flex-3 relative border-b border-white/5 flex flex-col">
          ...
          <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start z-30 bg-linear-to-b from-black/80 to-transparent">
            <div className="flex items-center gap-3">
              <div className={`px-2 py-1 rounded-sm text-[10px] font-bold uppercase tracking-wider border backdrop-blur-md ${activeTaskDetail?.status === 'processing' ? 'bg-amber-500/20 border-amber-500/30 text-amber-300' :
                activeTaskDetail?.status === 'completed' ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-300' :
                  'bg-zinc-800/50 border-white/10 text-zinc-500'
                }`}>
                {activeTaskDetail?.status || 'IDLE'}
              </div>
              {activeTaskDetail?.status === 'processing' && <span className="text-xs text-zinc-400 animate-pulse font-mono">Rendering on Cluster...</span>}
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => activeTaskDetail && activeTaskDetail.status === 'completed' && setIsFullscreen(true)}
          ...
          <div className="flex-1 flex items-center justify-center p-8 overflow-hidden bg-[#030303]">
            {activeTaskDetail ? (
                ...
                {activeTaskDetail.status === 'processing' ? (
                  ...
                  <div className="aspect-video w-full max-w-4xl bg-zinc-900/30 border border-white/5 rounded-sm flex flex-col items-center justify-center relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                      ...
                      <h3 className="text-lg font-medium text-white tracking-tight uppercase font-mono">Generating Scene</h3>
                      <p className="text-xs text-zinc-500 mt-2 font-mono uppercase tracking-widest">Estimating physics & lighting...</p>
                    </div>
                  </div>
                  ...
                  <div className="text-center text-red-400 space-y-4">
                    ...
                    <div className="font-mono text-xs uppercase tracking-widest">Generation Failed</div>
                  </div>
                ) : (
                  // Completed State
                  <div className="relative w-full max-w-4xl aspect-video bg-black rounded-sm overflow-hidden shadow-2xl border border-zinc-800 group">
                    {activeTaskDetail.outputs && activeTaskDetail.outputs.length > 0 && (
                      activeTaskDetail.outputs[0].type === 'video' ? (
                        ...
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={activeTaskDetail.outputs[0].url} alt="" className="w-full h-full object-contain" />
                      )
                    )}
                    {!activeTaskDetail.outputs?.length && (
                      <div className="flex items-center justify-center h-full text-zinc-500 font-mono text-xs">No media output available</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="text-center space-y-6 opacity-30 select-none">
                ...
                <h3 className="text-xl font-light text-white tracking-tight uppercase font-mono">Ready to Create</h3>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-2 bg-[#08080A] border-t border-white/5 flex flex-col min-h-0">
          <div className="p-4 border-b border-white/5 flex justify-between items-center bg-zinc-900/50 backdrop-blur z-10">
            <div className="flex items-center gap-3">
              ...
              <span className="px-1.5 py-0.5 rounded-sm bg-white/10 text-[9px] font-mono text-zinc-300">{tasks.length}</span>
            </div>
            <button onClick={() => fetchTasks()} className="text-zinc-500 hover:text-white transition-colors"><RefreshCcw className="w-3.5 h-3.5" /></button>
          </div>

          <div ref={historyListRef} className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3 scroll-smooth">
            {isLoadingTasks && (
              <div className="text-center py-8 text-zinc-600 font-mono text-xs animate-pulse">SYNCING DATABASE...</div>
            )}

            {!isLoadingTasks && tasks.length === 0 && (
              <div className="text-center py-8 text-zinc-600 font-mono text-xs uppercase">No history found</div>
            )}
            ...
            {tasks.map((task) => (
              <div
                ...
                className={`
                   ...
                   `}
              >
                {activeTaskId === task.id && <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-indigo-500"></div>}
                ...
                <div className="w-24 h-16 bg-black rounded-sm overflow-hidden shrink-0 relative border border-white/5 flex items-center justify-center">
                  ...
                  <div className="absolute bottom-0 right-0 p-1 bg-black/60 backdrop-blur-sm">
                    {task.type === TaskType.VIDEO_MOTION && <Activity className="w-3 h-3 text-zinc-400" />}
                    {task.type === TaskType.VIDEO_GENERATION && <Film className="w-3 h-3 text-zinc-400" />}
                    {task.type === TaskType.VIDEO_LIPSYNC && <Mic className="w-3 h-3 text-zinc-400" />}
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center py-0.5">
                  <div className="flex justify-between items-start mb-1">
                    <span className={`text-[9px] font-bold uppercase tracking-wider ${task.status === 'processing' ? 'text-amber-400' :
                      task.status === 'completed' ? 'text-emerald-400' :
                        task.status === 'failed' ? 'text-red-400' : 'text-zinc-500'
                      }`}>
                      {task.status === 'processing' ? 'Generating...' : task.status}
                    </span>
                    <span className="text-[9px] text-zinc-600 font-mono">{new Date(task.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                ...
                <div className="flex flex-col justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity translate-x-2 group-hover:translate-x-0">
                  <button className="p-1.5 hover:bg-zinc-700 rounded-sm text-zinc-400 hover:text-white" title="Download">
                    <Download className="w-3 h-3" />
                  </button>
                  <button className="p-1.5 hover:bg-red-900/30 rounded-sm text-zinc-400 hover:text-red-400" title="Delete">
