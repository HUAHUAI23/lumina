...
const VideoStudio: React.FC = () => {
  ...
  const handleGenerate = async () => {
    // Note: Mock service doesn't need API key check
                    ...
                    {analysisResult.extractedFrames.map((frame, i) => (
                      <React.Fragment key={i}>
                        <Image
                          ...
                          width={64}
                          height={64}
                          className="object-cover rounded-lg border border-indigo-500/20 shadow-sm"
                        />
                      </React.Fragment>
                    ))}
                  </div>
        ...
        {generatedVideoUrl ? (
          ...
          <div className="text-center space-y-6 z-10 opacity-60">
            <div className="w-24 h-24 rounded-full bg-zinc-900/50 border border-zinc-800 flex items-center justify-center mx-auto shadow-[0_0_30px_rgba(99,102,241,0.1)]">
              {/* Note: PlayCircle is imported as PlayCircleIcon in some libraries, using generic PlayCircle from lucide-react */}
