# Lumina AI Studio 认证系统设计文档

> **版本**: 1.0.0
> **日期**: 2025年12月
> **基于**: Next.js 16 + Drizzle ORM + PostgreSQL

## 目录

1. [系统概述](#系统概述)
2. [架构设计](#架构设计)
3. [安全最佳实践 (2025)](#安全最佳实践-2025)
4. [数据库设计](#数据库设计)
5. [API 路由](#api-路由)
6. [用户体验设计](#用户体验设计)
7. [部署指南](#部署指南)

---

## 系统概述

### 功能特性

✅ **已实现**：
- 用户名/密码登录注册
- GitHub OAuth 登录
- 统一登录注册入口（智能识别）
- JWT 会话管理
- 初始积分配置
- Data Access Layer 安全模式

🚧 **已设计（未实现）**：
- 邮箱验证码登录
- 短信验证码登录
- Google OAuth 登录
- 认证方式禁用机制

### 技术栈

| 技术        | 版本   | 用途       |
| ----------- | ------ | ---------- |
| Next.js     | 16.0.5 | 应用框架   |
| React       | 19.2.0 | UI 框架    |
| Drizzle ORM | 0.44.7 | 数据库 ORM |
| PostgreSQL  | -      | 数据库     |
| Jose        | 6.1.2  | JWT 处理   |
| bcryptjs    | 3.0.3  | 密码加密   |
| Zod         | 4.1.13 | 数据验证   |

---

## 架构设计

### 整体架构

```
┌─────────────────────────────────────────────────────────────┐
│                         用户界面                              │
│                    /login (统一入口)                          │
└──────────────┬──────────────────────────────────────────────┘
               │
┌──────────────▼──────────────────────────────────────────────┐
│                         API 层                                │
│  /api/auth/check         - 检查用户是否存在                   │
│  /api/auth/authenticate  - 统一认证接口                       │
│  /api/auth/github        - GitHub OAuth                      │
│  /api/auth/logout        - 登出                              │
│  /api/auth/me            - 获取当前用户                       │
└──────────────┬──────────────────────────────────────────────┘
               │
┌──────────────▼──────────────────────────────────────────────┐
│                      服务层 (Service Layer)                   │
│  lib/auth/service.ts - 业务逻辑                               │
│    - registerUser()         - 注册用户                       │
│    - loginWithPassword()    - 密码登录                       │
│    - findOrCreateGithubUser() - GitHub 登录                  │
└──────────────┬──────────────────────────────────────────────┘
               │
┌──────────────▼──────────────────────────────────────────────┐
│              数据访问层 (Data Access Layer)                   │
│  lib/auth/dal.ts - 认证验证 (2025 安全模式)                   │
│    - verifySession()    - 验证会话                           │
│    - getCurrentUserId() - 获取当前用户 ID                     │
│    - isAuthenticated()  - 检查是否认证                       │
└──────────────┬──────────────────────────────────────────────┘
               │
┌──────────────▼──────────────────────────────────────────────┐
│                 会话管理 (Session Management)                 │
│  lib/auth/session.ts - JWT 会话处理                           │
│    - createSession()  - 创建会话                             │
│    - getSession()     - 获取会话                             │
│    - deleteSession()  - 删除会话                             │
│    - updateSession()  - 更新会话                             │
└──────────────┬──────────────────────────────────────────────┘
               │
┌──────────────▼──────────────────────────────────────────────┐
│                    数据库 (PostgreSQL)                        │
│  - users                - 用户表                             │
│  - user_identities      - 用户身份表                         │
│  - accounts             - 账户表                             │
│  - verification_codes   - 验证码表 (未使用)                  │
└───────────────────────────────────────────────────────────────┘
```

### 认证流程

#### 密码登录/注册流程

```
用户输入用户名/邮箱
      │
      ▼
调用 /api/auth/check
      │
      ├─→ 用户存在 → 显示密码输入框 (登录模式)
      │                  │
      │                  ▼
      │            调用 /api/auth/authenticate (login)
      │                  │
      │                  ▼
      │            验证密码 → 创建会话 → 跳转首页
      │
      └─→ 用户不存在 → 显示注册表单 (注册模式)
                         │
                         ▼
                   调用 /api/auth/authenticate (register)
                         │
                         ▼
                   创建用户 → 创建账户 → 创建会话 → 跳转首页
```

#### GitHub OAuth 流程

```
点击 "Github" 按钮
      │
      ▼
跳转到 /api/auth/github
      │
      ▼
生成 state + 跳转到 GitHub 授权页面
      │
      ▼
用户授权 → GitHub 回调 /api/auth/github/callback
      │
      ├─→ 验证 state (CSRF 保护)
      │
      ├─→ 用 code 换取 access_token
      │
      ├─→ 获取 GitHub 用户信息
      │
      ├─→ 查找或创建用户
      │
      └─→ 创建会话 → 跳转首页
```

---

## 安全最佳实践 (2025)

### 重要安全更新

⚠️ **CVE-2025-29927** - Next.js Proxy 漏洞
- **影响**: Proxy (middleware) 不再被认为是安全的唯一防线
- **解决方案**: 实现 Data Access Layer 模式

### 多层安全防护

```
┌─────────────────────────────────────────────────────────┐
│  第1层: Proxy (proxy.ts)                                 │
│  - 初步检查会话                                           │
│  - 重定向未认证用户到登录页                               │
│  - 刷新会话（延长登录时间）                               │
└─────────────────────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────────────────────┐
│  第2层: API Route Handler                                │
│  - 处理请求                                               │
│  - 基本输入验证（Zod schema）                             │
└─────────────────────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────────────────────┐
│  第3层: Data Access Layer ⭐ 核心安全层                   │
│  - verifySession() 验证会话                               │
│  - 检查会话是否过期                                       │
│  - 返回用户信息或抛出异常                                 │
└─────────────────────────────────────────────────────────┘
```

### JWT 会话安全

- **算法**: HS256 (HMAC with SHA-256)
- **签名密钥**: `AUTH_SECRET` 环境变量（最少 32 字符）
- **会话时长**: 7 天
- **存储方式**: HTTP-only Cookie
- **Cookie 配置**:
  ```typescript
  {
    httpOnly: true,           // 防止 XSS
    secure: production,       // 生产环境仅 HTTPS
    sameSite: 'lax',          // 防止 CSRF
    expires: 7days,           // 自动过期
    path: '/'                 // 全站可用
  }
  ```

### 密码安全

- **加密算法**: bcrypt
- **Salt Rounds**: 10
- **密码要求**:
  - 最少 8 字符
  - 至少1个大写字母
  - 至少1个小写字母
  - 至少1个数字

### OAuth 安全

- **CSRF 保护**: 使用 `state` 参数
- **Token 存储**: 存储在数据库 `metadata.oauth.accessToken`
- **权限范围**: 最小权限原则 (`read:user`, `user:email`)

---

## 数据库设计

### ER 图

```
┌──────────────────────┐
│      users           │
│──────────────────────│
│  id (PK)             │
│  username (UNIQUE)   │
│  email (UNIQUE)      │
│  avatar              │
│  created_at          │
│  updated_at          │
└──────────┬───────────┘
           │ 1
           │
           │ N
┌──────────▼───────────┐        ┌─────────────────────┐
│  user_identities     │        │    accounts         │
│──────────────────────│        │─────────────────────│
│  id (PK)             │   1:1  │  id (PK)            │
│  user_id (FK)        │◄───────┤  user_id (FK,UNIQ)  │
│  provider (ENUM)     │        │  balance (BIGINT)   │
│  provider_user_id    │        │  created_at         │
│  metadata (JSONB)    │        │  updated_at         │
│  is_primary          │        └─────────────────────┘
│  created_at          │
│  updated_at          │
│  UNIQUE(provider,    │
│    provider_user_id) │
└──────────────────────┘

┌──────────────────────────┐
│  verification_codes      │  (预留，未使用)
│──────────────────────────│
│  id (PK)                 │
│  recipient               │
│  channel (ENUM)          │
│  code_hash               │
│  purpose                 │
│  expires_at              │
│  used                    │
│  attempts                │
│  max_attempts            │
│  user_id (FK, NULLABLE)  │
│  created_at              │
└──────────────────────────┘
```

### 表详细设计

#### users 表

用户基本信息表。

| 字段       | 类型         | 约束             | 说明         |
| ---------- | ------------ | ---------------- | ------------ |
| id         | serial       | PRIMARY KEY      | 用户ID       |
| username   | varchar(64)  | NOT NULL, UNIQUE | 用户名       |
| email      | varchar(255) | UNIQUE           | 邮箱（可选） |
| avatar     | text         | DEFAULT ''       | 头像 URL     |
| created_at | timestamptz  | NOT NULL         | 创建时间     |
| updated_at | timestamptz  | NOT NULL         | 更新时间     |

#### user_identities 表

用户身份认证方式表（支持多种登录方式）。

| 字段             | 类型          | 约束                    | 说明               |
| ---------------- | ------------- | ----------------------- | ------------------ |
| id               | serial        | PRIMARY KEY             | 身份ID             |
| user_id          | integer       | NOT NULL, FK → users.id | 所属用户           |
| provider         | auth_provider | NOT NULL                | 认证提供商         |
| provider_user_id | varchar(128)  | NOT NULL                | 提供商的用户ID     |
| metadata         | jsonb         | NOT NULL                | 认证元数据         |
| is_primary       | boolean       | DEFAULT false           | 是否为主要认证方式 |
| created_at       | timestamptz   | NOT NULL                | 创建时间           |
| updated_at       | timestamptz   | NOT NULL                | 更新时间           |

**索引**:
- UNIQUE(provider, provider_user_id) - 防止重复绑定
- INDEX(user_id, provider) - 查询优化

**metadata 结构**:
```typescript
{
  // 密码认证
  password?: {
    passwordHash: string      // bcrypt 哈希
    needReset?: boolean       // 是否需要重置密码
  }

  // OAuth 认证
  oauth?: {
    accessToken?: string      // OAuth access token
    refreshToken?: string     // OAuth refresh token
    email?: string            // OAuth 提供商返回的邮箱
    avatarUrl?: string        // OAuth 提供商返回的头像
    profile?: Record<string, unknown>  // 其他 profile 信息
  }
}
```

#### accounts 表

用户账户表（积分、余额等）。

| 字段       | 类型        | 约束                            | 说明     |
| ---------- | ----------- | ------------------------------- | -------- |
| id         | serial      | PRIMARY KEY                     | 账户ID   |
| user_id    | integer     | NOT NULL, UNIQUE, FK → users.id | 所属用户 |
| balance    | bigint      | NOT NULL, DEFAULT 0             | 积分余额 |
| created_at | timestamptz | NOT NULL                        | 创建时间 |
| updated_at | timestamptz | NOT NULL                        | 更新时间 |

**注意**:
- `balance` 使用 `bigint` 类型但配置为 `mode: 'number'`
- 适用于不超过 2^53 的数值（约 9 千万亿积分）
- 如需更大范围，使用 `mode: 'bigint'`

#### verification_codes 表（预留）

通用验证码表，支持多渠道（邮箱、短信、语音、WhatsApp）。

| 字段         | 类型                 | 约束                    | 说明                  |
| ------------ | -------------------- | ----------------------- | --------------------- |
| id           | serial               | PRIMARY KEY             | 验证码ID              |
| recipient    | varchar(255)         | NOT NULL                | 接收方（邮箱/手机）   |
| channel      | verification_channel | NOT NULL                | 投递渠道              |
| code_hash    | varchar(255)         | NOT NULL                | 验证码哈希            |
| purpose      | varchar(50)          | NOT NULL                | 用途 (login/register) |
| expires_at   | timestamptz          | NOT NULL                | 过期时间 (≤5分钟)     |
| used         | boolean              | DEFAULT false           | 是否已使用            |
| attempts     | integer              | DEFAULT 0               | 尝试次数              |
| max_attempts | integer              | DEFAULT 5               | 最大尝试次数          |
| user_id      | integer              | NULLABLE, FK → users.id | 关联用户              |
| created_at   | timestamptz          | NOT NULL                | 创建时间              |

**索引**:
- INDEX(recipient, channel)
- INDEX(expires_at)

### 枚举类型

```sql
-- 认证提供商
CREATE TYPE auth_provider AS ENUM (
  'password',  -- 用户名密码
  'google',    -- Google OAuth
  'github',    -- GitHub OAuth
  'email',     -- 邮箱验证码
  'sms'        -- 短信验证码
);

-- 验证码渠道
CREATE TYPE verification_channel AS ENUM (
  'email',     -- 邮箱
  'sms',       -- 短信
  'voice',     -- 语音
  'whatsapp'   -- WhatsApp
);
```

---

## API 路由

### API 响应规范

所有 API 遵循统一的响应格式 `ApiResponse<T>`（定义在 `lib/api-response.ts`）：

**成功响应**:
```typescript
{
  success: true
  data: T                    // 实际数据
  message?: string           // 可选的成功消息
}
```

**错误响应**:
```typescript
{
  success: false
  error: string              // 错误消息
  details?: unknown          // 可选的错误详情
}
```

### 认证相关

#### POST /api/auth/check

检查用户名/邮箱是否已存在（用于统一登录注册流程）。

**请求**:
```json
{
  "usernameOrEmail": "john_doe"
}
```

**成功响应**:
```json
{
  "success": true,
  "data": {
    "exists": true,
    "needsPassword": true,
    "needsRegistration": false
  }
}
```

**错误响应**:
```json
{
  "success": false,
  "error": "Invalid username or email format"
}
```

#### POST /api/auth/authenticate

统一认证接口，根据用户是否存在自动处理登录或注册。

**请求（登录）**:
```json
{
  "usernameOrEmail": "john_doe",
  "password": "Password123"
}
```

**请求（注册）**:
```json
{
  "usernameOrEmail": "john@example.com",
  "password": "Password123",
  "username": "john_doe",
  "email": "john@example.com"
}
```

**成功响应**:
```json
{
  "success": true,
  "data": {
    "action": "login",  // or "register"
    "user": {
      "id": 1,
      "username": "john_doe",
      "email": "john@example.com"
    }
  }
}
```

**错误响应**:
```json
{
  "success": false,
  "error": "Invalid credentials"
}
```

#### GET /api/auth/github

启动 GitHub OAuth 流程。

**行为**:
1. 生成 CSRF state
2. 跳转到 GitHub 授权页面

#### GET /api/auth/github/callback

GitHub OAuth 回调处理。

**查询参数**:
- `code`: GitHub 授权码
- `state`: CSRF state（验证）

**行为**:
1. 验证 state
2. 用 code 换取 access_token
3. 获取 GitHub 用户信息
4. 创建或查找用户
5. 创建会话
6. 跳转到首页

#### POST /api/auth/logout

登出用户。

**成功响应**:
```json
{
  "success": true,
  "message": "Logged out successfully"
}
```

**说明**: 登出接口无需返回 `data` 字段，只返回 `success` 和 `message`。

#### GET /api/auth/me

获取当前用户信息（需要认证）。

**成功响应**:
```json
{
  "success": true,
  "data": {
    "id": 1,
    "username": "john_doe",
    "email": "john@example.com",
    "avatar": "https://avatars.githubusercontent.com/...",
    "credits": 500
  }
}
```

**错误响应**:
```json
{
  "success": false,
  "error": "Unauthorized"
}
```

---

## 用户体验设计

### 统一登录注册入口

#### 设计理念

基于 2025 年 UX 最佳实践：
- **简化流程** - 用户无需判断是"登录"还是"注册"
- **智能识别** - 系统自动检测用户是否存在
- **渐进式表单** - 根据状态动态显示必要字段

#### 交互流程

```
┌─────────────────────────────────────┐
│   第一步：输入用户名或邮箱           │
│                                     │
│   Username or Email                 │
│   [_________________________]       │
│                                     │
│   [Continue] ──────────────────►    │
└─────────────────────────────────────┘
              │
              ├─→ 用户存在
              │   ┌─────────────────────────────────────┐
              │   │   第二步：登录模式                   │
              │   │                                     │
              │   │   Signing in as                     │
              │   │   [john_doe]           [Change]     │
              │   │                                     │
              │   │   Password                          │
              │   │   [_________________________]       │
              │   │                                     │
              │   │   [Sign In] ────────────────►       │
              │   └─────────────────────────────────────┘
              │
              └─→ 用户不存在
                  ┌─────────────────────────────────────┐
                  │   第二步：注册模式                   │
                  │                                     │
                  │   Username                          │
                  │   [john_doe_______________]         │
                  │                                     │
                  │   Email                             │
                  │   [john@example.com_______]         │
                  │                                     │
                  │   Password                          │
                  │   [_________________________]       │
                  │                                     │
                  │   Confirm Password                  │
                  │   [_________________________]       │
                  │                                     │
                  │   [Create Account] ─────────►       │
                  │   ← Back to start                   │
                  └─────────────────────────────────────┘
```

### OAuth 集成

在初始界面提供 OAuth 选项：

```
┌─────────────────────────────────────┐
│  Username or Email                  │
│  [_________________________]        │
│                                     │
│  [Continue] ──────────────────►     │
│                                     │
│  ────────── Or ──────────          │
│                                     │
│  [GitHub图标] Github                │
│  [Google图标] Google                │
└─────────────────────────────────────┘
```

---

## 部署指南

### 环境变量配置

复制 `.env.example` 为 `.env` 并配置：

```bash
# 数据库
DATABASE_URL=postgresql://user:password@host:5432/dbname

# 认证密钥 (至少 32 字符)
AUTH_SECRET=$(openssl rand -base64 32)

# GitHub OAuth (可选)
GITHUB_CLIENT_ID=your_github_client_id
GITHUB_CLIENT_SECRET=your_github_client_secret

# 环境
NODE_ENV=production
```

### 数据库迁移

```bash
# 1. 生成迁移文件
pnpm db:generate

# 2. 应用迁移
pnpm db:push

# 或使用 migrate
pnpm db:migrate
```

### 验证部署

1. **检查环境变量**:
   ```bash
   node -e "const {env} = require('./lib/env'); console.log('✅ 环境变量配置正确')"
   ```

2. **测试数据库连接**:
   ```bash
   pnpm db:studio
   ```

3. **测试登录功能**:
   - 访问 `/login`
   - 尝试注册新用户
   - 尝试登录
   - 测试 GitHub OAuth（如已配置）

---

## 参考资料

### 官方文档
- [Next.js 16 Documentation](https://nextjs.org/docs)
- [Drizzle ORM Documentation](https://orm.drizzle.team/)
- [Jose (JWT) Documentation](https://github.com/panva/jose)

### 安全最佳实践
- [Next.js Authentication Best Practices (2025)](https://nextjs.org/docs/pages/guides/authentication)
- [OWASP Authentication Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html)
- [CVE-2025-29927 - Next.js Middleware Vulnerability](https://medium.com/@franciscomoretti/next-js-authentication-best-practices-6897dad3a170)

### UX 设计
- [Login & Signup UX – Complete 2025 Guide](https://www.authgear.com/post/login-signup-ux-guide)
- [Best Sign Up Flows (2025)](https://www.eleken.co/blog-posts/sign-up-flow)

---

## 版本历史

| 版本  | 日期    | 变更内容                                     |
| ----- | ------- | -------------------------------------------- |
| 1.0.0 | 2025-12 | 初始版本 - 实现用户名密码登录和 GitHub OAuth |

---

## 维护者

Lumina AI Studio Team

如有问题或建议，请提交 Issue。
